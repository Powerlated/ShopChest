...
public class ChestProtectListener implements Listener {
			...
			Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
				...
				public void run() {
					...
					if (b.getLocation().equals(l.getLocation()))
						newShop = new Shop(shop.getID(), plugin, shop.getVendor(), shop.getProduct(), r.getLocation(),
								shop.getBuyPrice(), shop.getSellPrice(), shop.getShopType());
					else if (b.getLocation().equals(r.getLocation()))
						newShop = new Shop(shop.getID(), plugin, shop.getVendor(), shop.getProduct(), l.getLocation(),
								shop.getBuyPrice(), shop.getSellPrice(), shop.getShopType());
	...
	public void onBlockBreak(BlockBreakEvent e) {
		...
		if (shopUtils.isShop(b.getLocation())) {
			...
			if (p.isSneaking()) {
				plugin.debug(String.format("%s tries to break %s's shop (#%d)", p.getName(), shop.getVendor().getName(),
						shop.getID()));
		...
		if (b.getType().equals(Material.CHEST) || b.getType().equals(Material.TRAPPED_CHEST)) {
			...
			if (ih instanceof DoubleChest) {
				...
				if (shopUtils.isShop(r.getLocation()) || shopUtils.isShop(l.getLocation())) {
					...
					plugin.debug(String.format("%s tries to extend %s's shop (#%d)", p.getName(),
							shop.getVendor().getName(), shop.getID()));
					...
					if (externalPluginsAllowed || p.hasPermission(Permissions.EXTEND_PROTECTED)) {
						if (shop.getVendor().getUniqueId().equals(p.getUniqueId())
								|| p.hasPermission(Permissions.EXTEND_OTHER)) {

							if (b.getRelative(BlockFace.UP).getType() == Material.AIR) {
								shopUtils.removeShop(shop, true);
								Shop newShop = new Shop(shop.getID(), ShopChest.getInstance(), shop.getVendor(),
										shop.getProduct(), shop.getLocation(), shop.getBuyPrice(), shop.getSellPrice(),
										shop.getShopType());
								shopUtils.addShop(newShop, true);
								plugin.debug(String.format("%s extended %s's shop (#%d)", p.getName(),
										shop.getVendor().getName(), shop.getID()));
							} else {
								e.setCancelled(true);
							...
							e.setCancelled(true);
							p.sendMessage(
									LanguageUtils.getMessage(LocalizedMessage.Message.NO_PERMISSION_EXTEND_OTHERS));
						}
					} else {
						e.setCancelled(true);
						p.sendMessage(
								LanguageUtils.getMessage(LocalizedMessage.Message.NO_PERMISSION_EXTEND_PROTECTED));
					}
	...
	public void onItemMove(InventoryMoveItemEvent e) {
		if (plugin.getShopChestConfig().hopper_protection) {
			if ((e.getSource().getType().equals(InventoryType.CHEST))
					&& (!e.getInitiator().getType().equals(InventoryType.PLAYER))) {
				...
				if (e.getSource().getHolder() instanceof DoubleChest) {
					...
					if (shopUtils.isShop(c.getLocation()))
						e.setCancelled(true);
				}
