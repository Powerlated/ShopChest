...
public class ShopChest extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (worldGuard != null && !ShopFlag.isLoaded()) {
			...
			try {
				...
				getLogger().severe(
						"Failed to reload WorldGuard region manager. WorldGuard support will probably not work!");
			...
			Bukkit.getScheduler().runTaskTimer(this, new Runnable() {
				...
				public void run() {
					...
					if (!event.isCancelled())
						shopUtils.reloadShops(true, false);
				}
		...
		Bukkit.getScheduler().runTaskAsynchronously(this, new Runnable() {
			...
			public void run() {
				...
				Bukkit.getConsoleSender().sendMessage(
						"[ShopChest] " + LanguageUtils.getMessage(LocalizedMessage.Message.UPDATE_CHECKING));
				if (result == UpdateCheckerResult.TRUE) {
					latestVersion = uc.getVersion();
					downloadLink = uc.getLink();
					isUpdateNeeded = true;
					Bukkit.getConsoleSender().sendMessage(
							"[ShopChest] " + LanguageUtils.getMessage(LocalizedMessage.Message.UPDATE_AVAILABLE,
									new LocalizedMessage.ReplacedRegex(Regex.VERSION, latestVersion)));

					for (Player p : getServer().getOnlinePlayers()) {
						if (p.hasPermission(Permissions.UPDATE_NOTIFICATION)) {
							JsonBuilder jb = new JsonBuilder(ShopChest.this,
					...
					Bukkit.getConsoleSender().sendMessage(
							"[ShopChest] " + LanguageUtils.getMessage(LocalizedMessage.Message.UPDATE_NO_UPDATE));
				} else {
					latestVersion = "";
					downloadLink = "";
					isUpdateNeeded = false;
					Bukkit.getConsoleSender().sendMessage(
							"[ShopChest] " + LanguageUtils.getMessage(LocalizedMessage.Message.UPDATE_ERROR));
				}
			}
		});
		...
		try {
			...
			ShopCommand.registerCommand(
					new ShopCommand(this, config.main_command_name, "Manage Shops.", "", new ArrayList<String>()),
					this);
		} catch (Exception e) {
	 ...
	 * <i>/plugins/ShopChest/debug.txt</i> file
	 * 
	 * @param throwable
	 *            {@link Throwable} whose stacktrace will be printed
	 */
	...
	/**
	 * @return An instance of {@link Towny} or {@code null} if Towny is not
	 *         enabled
	 */
	...
	/**
	 * @return An instance of {@link WorldGuardPlugin} or {@code null} if
	 ...
	 *            Whether an update should be needed
	 */
	...
	/**
	 * @return The latest version of ShopChest (will return null if not checked
	 ...
	 * 
	 * @param latestVersion
	 *            Version to set as latest version
	 */
	...
	/**
	 * @return The download link of the latest version (will return null if not
	 *         checked or if no update is available)
	 */
	...
	/**
	 * Set the download Link of the latest version (will return null if not
	 * checked or if no update is available)
	 * 
	 * @param downloadLink
	 *            Link to set as Download Link
	 */
