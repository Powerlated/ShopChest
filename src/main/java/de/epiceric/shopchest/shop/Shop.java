...
public class Shop {
	...
	public Shop(int id, ShopChest plugin, OfflinePlayer vendor, ItemStack product, Location location, double buyPrice,
			double sellPrice, ShopType shopType) {
		this.id = id;
		this.plugin = plugin;
		this.vendor = vendor;
		...
		if (b.getType() != Material.CHEST && b.getType() != Material.TRAPPED_CHEST) {
			...
			plugin.getLogger()
					.severe("No Chest found at specified Location: " + b.getX() + "; " + b.getY() + "; " + b.getZ());
			...
			return;
				...
				&& plugin.getShopChestConfig().show_shop_items) {
			plugin.getShopUtils().removeShop(this, plugin.getShopChestConfig().remove_shop_on_error);
			plugin.getLogger().severe(
					"No space above chest at specified Location: " + b.getX() + "; " + b.getY() + "; " + b.getZ());
	...
	private Shop(OfflinePlayer vendor, ItemStack product, Location location, double buyPrice, double sellPrice,
			ShopType shopType) {
		this.id = 0;
		this.vendor = vendor;
		this.product = product;
	 ...
	 * </p>
	 * <b>Call this after {@link #createHologram()}, because it depends on the
	 * hologram's location</b>
	 */
	private void createItem() {
		if (plugin.getShopChestConfig().show_shop_items) {
			...
			itemLocation = new Location(location.getWorld(), hologram.getLocation().getX(), location.getY() + 1,
	...
	/**
	 * @return {@link InventoryHolder} of the shop or <b>null</b> if the shop
	 *         has no chest.
	 */
	...
	/**
	 * @return A shop, which is not really a shop. It's just for "storing" the
	 *         data (used in some events).
	 */
	public static Shop createImaginaryShop(OfflinePlayer vendor, ItemStack product, Location location, double buyPrice,
			double sellPrice, ShopType shopType) {
