...
public class ShopItem {
	...
	public ShopItem(ShopChest plugin, ItemStack itemStack, Location location) {
		...
		Class[] requiredClasses = new Class[] { nmsWorldClass, craftWorldClass, nmsItemStackClass, craftItemStackClass,
				entityItemClass, packetPlayOutSpawnEntityClass, packetPlayOutEntityMetadataClass, dataWatcherClass,
	...
	private void create() {
		try {
			...
			entityItemClass.getMethod("setPosition", double.class, double.class, double.class).invoke(entityItem,
			...
			if (Utils.getMajorVersion() >= 10)
				entityItemClass.getMethod("setNoGravity", boolean.class).invoke(entityItem, true);
			...
			creationPackets[0] = packetPlayOutSpawnEntityClass.getConstructor(entityClass, int.class)
					.newInstance(entityItem, 2);
			creationPackets[1] = packetPlayOutEntityMetadataClass
					.getConstructor(int.class, dataWatcherClass, boolean.class)
					.newInstance(entityId, dataWatcher, true);
			creationPackets[2] = packetPlayOutEntityVelocityClass
					.getConstructor(int.class, double.class, double.class, double.class)
					.newInstance(entityId, 0D, 0D, 0D);
	 ...
	 * 
	 * @param p
	 *            Player, for which the item should be reset
	 */
	...
	public void setVisible(final Player p, boolean visible) {
		...
		if (visible) {
			...
			try {
				if (p.isOnline()) {
					Object packetPlayOutEntityDestroy = packetPlayOutEntityDestroyClass.getConstructor(int[].class)
	...
	/**
	 * @return Clone of the location, where the shop item should be (it could
	 *         have been moved by something, even though it shouldn't) To get
	 *         the exact location, use reflection and extract the location of
	 *         the {@code EntityItem} which you can get in
	 *         {@link #getEntityItem()}.
	 */
