...
public class ShopFlag {
	...
	static {
		CREATE_SHOP = new StateFlag("create-shop",
				ShopChest.getInstance().getShopChestConfig().wg_allow_create_shop_default);
		USE_SHOP = new StateFlag("use-shop", ShopChest.getInstance().getShopChestConfig().wg_allow_use_shop_default);
		USE_ADMIN_SHOP = new StateFlag("use-admin-shop",
				ShopChest.getInstance().getShopChestConfig().wg_allow_use_admin_shop_default);
	...
	public static void init(final ShopChest plugin, boolean onLoad) {
		...
		if (((majorVersion == 6 && minorVersion == 1 && fixVersion >= 3) || (majorVersion == 6 && minorVersion > 1)
				|| majorVersion > 6)) {
			if (onLoad) {
