...
public class JsonBuilder {
	...
	public JsonBuilder(ShopChest plugin, String text, String hoverText, String downloadLink) {
		...
		Class[] requiredClasses = new Class[] { iChatBaseComponentClass, packetPlayOutChatClass, chatSerializerClass };
		...
		for (String word : words) {
			try {
				if (index != words[0].length())
					withText(word).withColor("§" + text.charAt(index - 1)).withHoverEvent(hoverText)
							.withClickEvent(downloadLink);
			} catch (Exception e) {
			}
			index += word.length() + 2;
		}
	...
	private JsonBuilder withClickEvent(String value) {
		addSegment("\"clickEvent\":{\"action\":\"open_url" + "\",\"value\":\"" + value + "\"}");
		return this;
	}

	private JsonBuilder withHoverEvent(String value) {
		addSegment("\"hoverEvent\":{\"action\":\"show_text" + "\",\"value\":\"" + value + "\"}");
		return this;
	}
	...
	public void sendJson(Player p) {
		try {
			...
			Object packetPlayOutChat = packetPlayOutChatClass.getConstructor(iChatBaseComponentClass)
