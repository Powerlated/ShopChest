...
public class SpawnEggMeta {
	...
	private static String getNBTEntityID(ShopChest plugin, ItemStack stack) {
		try {
			...
			Object entityTagCompound = nbtTagCompound.getClass().getMethod("getCompound", String.class)
					.invoke(nbtTagCompound, "EntityTag");
			if (entityTagCompound == null)
				return null;

			Object id = entityTagCompound.getClass().getMethod("getString", String.class).invoke(entityTagCompound,
					"id");
	...
	/**
	 * @param stack
	 *            {@link ItemStack} (Spawn Egg) of which the Entity should be
	 *            gotten
	 * @return The {@link EntityType} the Spawn Egg will spawn or <b>null</b> if
	 *         <i>nbtEntityID</i> is null
	 */
