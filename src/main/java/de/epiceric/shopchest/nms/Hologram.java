...
public class Hologram {
	...
	public Hologram(ShopChest plugin, String[] text, Location location) {
		...
		Class[] requiredClasses = new Class[] { nmsWorldClass, entityArmorStandClass, entityLivingClass, entityClass,
	...
	private void create() {
		...
		for (int i = 0; i <= text.length; i++) {
			...
			try {
				...
				Constructor entityArmorStandConstructor = entityArmorStandClass.getConstructor(nmsWorldClass,
						double.class, double.class, double.class);
				Object entityArmorStand = entityArmorStandConstructor.newInstance(nmsWorldServer, loc.getX(),
						loc.getY(), loc.getZ());

				if (text != null) {
					entityArmorStandClass.getMethod("setCustomName", String.class).invoke(entityArmorStand, text);
					entityArmorStandClass.getMethod("setCustomNameVisible", boolean.class).invoke(entityArmorStand,
							true);
				}
				...
				Method addEntityMethod = nmsWorldServer.getClass()
						.getDeclaredMethod((Utils.getMajorVersion() == 8 ? "a" : "b"), entityClass);
	...
	/**
	 * @param p
	 *            Player to which the hologram should be shown
	 */
	public void showPlayer(Player p) {
		for (Object o : entityList) {
			try {
				Object entityLiving = entityLivingClass.cast(o);
				Object packet = packetPlayOutSpawnEntityLivingClass.getConstructor(entityLivingClass)
						.newInstance(entityLiving);
	...
	/**
	 * @param p
	 *            Player from which the hologram should be hidden
	 */
	...
	/**
	 * @param armorStand
	 *            Armor stand that's part of a hologram
