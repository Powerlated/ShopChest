...
public class Config {
	...
	/**
	 * The quality of hologram and item updating (performance saving, or better
	 * quality)
	 **/
	 ...
	 * <i>minimum-prices</i> section in ShopChest's config. To actually retrieve
	 * the minimum price for an item, you have to get the double
	 * {@code minimum-prices.<key>}.
	 **/
	 ...
	 * </p>
	 * This returns a key set, which contains e.g "STONE", "STONE:1", of the
	 * {@code maximum-prices} section in ShopChest's config. To actually
	 * retrieve the maximum price for an item, you have to get the double
	 * {@code maximum-prices.<key>}.
	 **/
	...
	/**
	 * Whether the buy price of a shop must be greater than or equal the sell
	 * price
	 **/
	...
	/**
	 * <p>
	 * Whether shops should automatically be removed from the database if an
	...
	/**
	 * Whether the item amount should be calculated to fit the available money
	 * or inventory space
	 **/
	...
	/**
	 * The default shop limit for players whose limit is not set via a
	 * permission
	 **/
	public int default_limit;

	/**
	 * The time between automatic shop reloads (if set to 0, the timer will be
	 * disabled)
	 **/
	...
	/**
	 * Add a value to a list in the config.yml. If the list does not exist, a
	 * new list with the given value will be created
	 * 
	 * @param property
	...
	public void add(String property, String value) {
		List list = (plugin.getConfig().getList(property) == null) ? new ArrayList<>()
				: plugin.getConfig().getList(property);
	...
	public void reload(boolean firstLoad, boolean langReload, boolean showMessages) {
		...
		wg_allow_use_admin_shop_default = plugin.getConfig()
				.getBoolean("worldguard-default-flag-values.use-admin-shop");
		...
		minimum_prices = (plugin.getConfig().getConfigurationSection("minimum-prices") == null) ? new HashSet<String>()
				: plugin.getConfig().getConfigurationSection("minimum-prices").getKeys(true);
		maximum_prices = (plugin.getConfig().getConfigurationSection("maximum-prices") == null) ? new HashSet<String>()
				: plugin.getConfig().getConfigurationSection("maximum-prices").getKeys(true);
		...
		auto_calculate_item_amount = (allow_decimals_in_price
				&& plugin.getConfig().getBoolean("auto-calculate-item-amount"));
		blacklist = (plugin.getConfig().getStringList("blacklist") == null) ? new ArrayList<String>()
				: plugin.getConfig().getStringList("blacklist");
	...
	private void loadLanguageConfig(boolean showMessages) {
		...
		if (!langConfigFile.exists()) {
				...
				try {
					...
					if (r == null) {
						...
						if (showMessages)
							plugin.getLogger()
									.info("Using locale \""
											+ langConfigFile.getName().substring(0,
													langConfigFile.getName().length() - 5)
											+ "\" (Streamed from jar file)");
					}
			...
			try {
				if (showMessages)
					plugin.getLogger().info("Using locale \""
							+ langConfigFile.getName().substring(0, langConfigFile.getName().length() - 5) + "\"");
